...
const modes = [
  { id: 'cards', label: 'Word sprint', icon: Zap },
  { id: 'quiz', label: 'Quick quiz', icon: Brain },
  { id: 'builder', label: 'Sentence lab', icon: BookOpen },
  { id: 'scene', label: 'Real-life scene', icon: MessagesSquare },
]
    ...
    <div className="page dashboard">
      ...
      <section className="continue-card">
        <div className="continue-copy">
          ...
          <h2>Your first week<br />in Barcelona</h2>
          <p>Continue your survival Spanish journey.</p>
          <button className="primary light-button" onClick={() => onOpen(packs[0])}>Continue <ArrowRight size={18} /></button>
        </div>
  ...
  return (
    <div className="page pack-page">
      ...
      <section className="mode-grid">
        {modes.map((mode, index) => { const IconMode = mode.icon; return (
          <button key={mode.id} className="mode-card" onClick={() => onStart(mode.id)}>
            ...
            <p>{['Flip through words in context, not lonely translations.', 'Fast choices with immediate, useful feedback.', 'Build Spanish with your hands and feel the word order.', 'Choose what you would actually say in a real moment.'][index]}</p>
...
function Cards({ pack, onAward, onFinish }) {
  ...
  return <ExerciseFrame kicker="MAKE IT STICK" title="Say it before you reveal it" current={index + 1} total={pack.words.length}>
    <button className={`flashcard ${flipped ? 'flipped' : ''}`} onClick={() => setFlipped(!flipped)}>
      <span className="card-label">{flipped ? 'MEANING & CONTEXT' : 'SPANISH'}</span><h2>{flipped ? word.en : word.es}</h2>
      {flipped ? <p>“{word.example}”</p> : <span className="flip-hint"><RotateCcw size={17} /> Tap to reveal</span>}
...
function Quiz({ pack, onAward, onFinish }) {
  ...
  const item = pack.quiz[index]; const correct = picked === item.answer
  const next = () => { if (correct) onAward(); if (index === pack.quiz.length - 1) onFinish(); else { setIndex(index + 1); setPicked(null) } }
  return <ExerciseFrame kicker="QUICK QUIZ" title={item.prompt} current={index + 1} total={pack.quiz.length}>
    ...
    <button className="primary lesson-next" disabled={!picked} onClick={next}>{index === pack.quiz.length - 1 ? 'Finish' : 'Next'} <ArrowRight size={18} /></button>
  </ExerciseFrame>
}
...
function Builder({ pack, onAward, onFinish }) {
  ...
  const item = pack.builder[index]; const shuffled = useMemo(() => [...item.answer].sort(() => 0.5 - Math.random()), [item]); const correct = chosen.join(' ') === item.answer.join(' ')
  const next = () => { if (correct) onAward(); if (index === pack.builder.length - 1) onFinish(); else { setIndex(index + 1); setChosen([]); setChecked(false) } }
  return <ExerciseFrame kicker="SENTENCE LAB" title={item.translation} current={index + 1} total={pack.builder.length}>
    ...
    {!checked ? <button className="primary lesson-next" disabled={!chosen.length} onClick={() => setChecked(true)}>Check sentence <Check size={18} /></button> : <button className="primary lesson-next" onClick={next}>{index === pack.builder.length - 1 ? 'Finish' : 'Next'} <ArrowRight size={18} /></button>}
  </ExerciseFrame>
}
